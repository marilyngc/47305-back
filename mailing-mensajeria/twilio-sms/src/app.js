...
app.post("/send-sms", async(req,res) => {
    try {
        const result = await twilioClient.messages.create({
            // intrucciones
            // desde donde se envia en mensaje
            from:config.twilio.phone,

            // aqui se lo mandamos
            to:"+541138326832",
            // indicamos que mensajes enviamos
            body:"TE AMO <3"
        });
        console.log(result);
        res.json({status:"succes",message:"mensaje enviado"});
    } catch (error) {
        console.log(error);
        res.json({status:"error", message:"Hubo un error al enviar el mensaje"})
    }
})